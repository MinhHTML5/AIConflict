function GraphicEngine () {
	...
	this.DrawTextRGB = function (context, text, x, y, w, font, size, bold, italic, alignW, alignH, red, green, blue, alpha, breakLine)
	{
		if (alpha == null) alpha = 1;
		if (alpha < 1) {
			context.globalAlpha = alpha * globalAlpha;
		...
		if (!found) {
			...
			if (navigator.isCocoonJS) {
				...
				var temp = new DrawTextRequest(text, w, font, size, bold, italic, alignW, alignH, red, green, blue, breakLine);
...
function DrawTextRequest (text, w, font, size, bold, italic, alignW, alignH, red, green, blue, breakLine) {
	this.m_text = text;
	this.m_w = w;
	this.m_font = font;
	this.m_size = size;
	...
	this.Compare = function (text, w, font, size, bold, italic, alignW, alignH, red, green, blue, breakLine) {
		if (this.m_text == text &&
			this.m_w == w &&
			this.m_font == font &&
			this.m_size == size &&
