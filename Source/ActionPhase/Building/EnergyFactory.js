function EnergyFactory (battle, x, y) {
	var baseImage = g_graphicEngine.LoadImage(ROOT_FOLDER + "Image/ActionPhase/Building/EnergyFactory/Base.png");
	
	this.m_type = ENUM_BUILDING_ENERGY;
	this.m_blockX = x;
	this.m_blockY = y;
	...
	this.m_name = g_turretData[ENUM_BUILDING_ENERGY].m_name;
	...
	this.Update = function (deltaTime) {
		if (battle.m_energy < MAX_ENERGY) {
			battle.m_energy += ENERGY_PRODUCE_RATE * deltaTime * 0.001;
			if (battle.m_energy > MAX_ENERGY) {
				battle.m_energy = MAX_ENERGY;
			}
		}
	}
