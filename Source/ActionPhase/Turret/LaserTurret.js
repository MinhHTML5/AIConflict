function LaserTurret (battle, x, y) {
	...
	var baseImage = g_graphicEngine.LoadImage(ROOT_FOLDER + "Image/ActionPhase/Turret/Laser/Base.png");
	var shadowImage = g_graphicEngine.LoadImage(ROOT_FOLDER + "Image/ActionPhase/Turret/Laser/Shadow.png");
	var turretImage = g_graphicEngine.LoadImage(ROOT_FOLDER + "Image/ActionPhase/Turret/Laser/Turret.png");
	
	this.m_type = ENUM_TURRET_LASER;
	this.m_blockX = x;
	this.m_blockY = y;
	...
	this.m_name = g_turretData[ENUM_TURRET_LASER].m_name;
	this.m_range = g_turretData[ENUM_TURRET_LASER].m_range;
	this.m_rotateSpeed = g_turretData[ENUM_TURRET_LASER].m_rotateSpeed;
	this.m_rateOfFire = g_turretData[ENUM_TURRET_LASER].m_rateOfFire;
	this.m_ammoCost = g_turretData[ENUM_TURRET_LASER].m_ammoCost;
	this.m_energyCost = g_turretData[ENUM_TURRET_LASER].m_energyCost;
	...
	g_laserSoundNode.SetCameraHandler (battle.camera);
	...
	this.DrawBullet = function (context) {
		...
		for (var i=0; i<this.bullet.length; i++) {
			if (this.bullet[i].m_x >= battle.camera.x - BATTLE_FIELD_BLOCK_SIZE - this.m_range     &&
				this.bullet[i].m_x <= battle.camera.x + battle.camera.w + BATTLE_FIELD_BLOCK_SIZE + this.m_range &&
				this.bullet[i].m_y >= battle.camera.y - BATTLE_FIELD_BLOCK_SIZE - this.m_range     &&
				this.bullet[i].m_y <= battle.camera.y + battle.camera.h + BATTLE_FIELD_BLOCK_SIZE + this.m_range ){
	...
	this.Attack = function () {
		if (rateOfFireCount <= 0) {
			var ammoCost = this.m_ammoCost;
			var energyCost = this.m_energyCost;
			
			if (g_profile.upgrade[UPGRADE_LASER][UPGRADE_TIER_2] == true) {
				ammoCost *= UPGRADE_LASER_POWER_CONSUMPTION_MULTIPLIER;
				energyCost *= UPGRADE_LASER_POWER_CONSUMPTION_MULTIPLIER;
			}
			
			if (battle.m_ammunition >= ammoCost && battle.m_energy >= energyCost) {
				battle.m_ammunition -= ammoCost;
				battle.m_energy -= energyCost;
				
				rateOfFireCount += this.m_rateOfFire;
	
				var temp = new LaserProjectile (battle, this.m_x, this.m_y, this.m_angle, this.m_target);
				this.bullet.push (temp);
				
				g_laserSoundNode.Play(this.m_x, this.m_y);
			}
		}
	}
	...
	this.SpecialAttack = function () {
		if (rateOfFireCount <= 0) {
			...
			if (battle.m_ammunition >= ammoCost && battle.m_energy >= energyCost) {
				...
				var temp = new SuperLaserProjectile (battle, this.m_x, this.m_y, this.m_angle, this.m_target);
				this.bullet.push (temp);
