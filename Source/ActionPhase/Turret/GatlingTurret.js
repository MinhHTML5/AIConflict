function GatlingTurret (battle, x, y) {
	...
	var baseImage = g_graphicEngine.LoadImage(ROOT_FOLDER + "Image/ActionPhase/Turret/Gatling/Base.png");
	var shadowImage = g_graphicEngine.LoadImage(ROOT_FOLDER + "Image/ActionPhase/Turret/Gatling/Shadow.png");
	var turretImage = g_graphicEngine.LoadImage(ROOT_FOLDER + "Image/ActionPhase/Turret/Gatling/Turret.png");
	
	this.m_type = ENUM_TURRET_GATLING;
	this.m_blockX = x;
	this.m_blockY = y;
	...
	this.m_name = g_turretData[ENUM_TURRET_GATLING].m_name;
	this.m_range = g_turretData[ENUM_TURRET_GATLING].m_range;
	this.m_rotateSpeed = g_turretData[ENUM_TURRET_GATLING].m_rotateSpeed;
	this.m_rateOfFire = g_turretData[ENUM_TURRET_GATLING].m_rateOfFire;
	this.m_ammoCost = g_turretData[ENUM_TURRET_GATLING].m_ammoCost;
	this.m_energyCost = g_turretData[ENUM_TURRET_GATLING].m_energyCost;
	...
	var recoilRate = 5;
	if (g_profile.upgrade[UPGRADE_GATLING][UPGRADE_TIER_1] == true) {
		recoilRate = UPGRADE_GATLING_RECOIL;
	}
	...
	g_gatlingSoundNode.SetCameraHandler (battle.camera);
	...
	this.DrawBullet = function (context) {
		...
		for (var i=0; i<this.bullet.length; i++) {
			if (this.bullet[i].m_x >= battle.camera.x - BATTLE_FIELD_BLOCK_SIZE     &&
				this.bullet[i].m_x <= battle.camera.x + battle.camera.w + BATTLE_FIELD_BLOCK_SIZE &&
				this.bullet[i].m_y >= battle.camera.y - BATTLE_FIELD_BLOCK_SIZE     &&
				this.bullet[i].m_y <= battle.camera.y + battle.camera.h + BATTLE_FIELD_BLOCK_SIZE ){
	...
	this.Attack = function () {
		if (rateOfFireCount <= 0) {
			if (battle.m_ammunition >= this.m_ammoCost && battle.m_energy >= this.m_energyCost) {
				...
				var recoil = recoilRate - (Math.random() * recoilRate * 2) >> 0
				var temp = new GatlingProjectile (battle, this.m_x, this.m_y, this.m_angle + recoil);
				this.bullet.push (temp);
				
				g_gatlingSoundNode.Play(this.m_x, this.m_y);
			}
		}
	}
	...
	this.SpecialAttack = function () {
		if (rateOfFireCount <= 0) {
			if (battle.m_ammunition >= this.m_ammoCost * 3 && battle.m_energy >= this.m_energyCost * 3) {
				battle.m_ammunition -= this.m_ammoCost * 3;
				battle.m_energy -= this.m_energyCost * 3;
				
				rateOfFireCount += this.m_rateOfFire * 3;
				
				var recoil = (recoilRate - (Math.random() * recoilRate * 2) >> 0);
				var temp = new GatlingProjectile (battle, this.m_x, this.m_y, this.m_angle - 10 + recoil);
				this.bullet.push (temp);
				temp = new GatlingProjectile (battle, this.m_x, this.m_y, this.m_angle + 0 + recoil);
				this.bullet.push (temp);
				temp = new GatlingProjectile (battle, this.m_x, this.m_y, this.m_angle + 10 + recoil);
				this.bullet.push (temp);
