function StaticTurret (battle, x, y) {
	...
	var baseImage = g_graphicEngine.LoadImage(ROOT_FOLDER + "Image/ActionPhase/Turret/Static/Base.png");
	var shadowImage = g_graphicEngine.LoadImage(ROOT_FOLDER + "Image/ActionPhase/Turret/Static/Shadow.png");
	var turretImage = g_graphicEngine.LoadImage(ROOT_FOLDER + "Image/ActionPhase/Turret/Static/Turret.png");
	
	this.m_type = ENUM_TURRET_STATIC;
	this.m_blockX = x;
	this.m_blockY = y;
	...
	this.m_name = g_turretData[ENUM_TURRET_STATIC].m_name;
	this.m_range = g_turretData[ENUM_TURRET_STATIC].m_range;
	this.m_rotateSpeed = g_turretData[ENUM_TURRET_STATIC].m_rotateSpeed;
	this.m_rateOfFire = g_turretData[ENUM_TURRET_STATIC].m_rateOfFire;
	this.m_ammoCost = g_turretData[ENUM_TURRET_STATIC].m_ammoCost;
	this.m_energyCost = g_turretData[ENUM_TURRET_STATIC].m_energyCost;
	...
	g_cannonSoundNode.SetCameraHandler (battle.camera);
	g_staticSoundNode.SetCameraHandler (battle.camera);
	...
	this.DrawBullet = function (context) {
		...
		for (var i=0; i<this.bullet.length; i++) {
			if (this.bullet[i].m_x >= battle.camera.x - BATTLE_FIELD_BLOCK_SIZE - this.bullet[i].m_areaOfEffect     &&
				this.bullet[i].m_x <= battle.camera.x + battle.camera.w + BATTLE_FIELD_BLOCK_SIZE + this.bullet[i].m_areaOfEffect &&
				this.bullet[i].m_y >= battle.camera.y - BATTLE_FIELD_BLOCK_SIZE - this.bullet[i].m_areaOfEffect     &&
				this.bullet[i].m_y <= battle.camera.y + battle.camera.h + BATTLE_FIELD_BLOCK_SIZE + this.bullet[i].m_areaOfEffect ){
	...
	this.Attack = function () {
		if (rateOfFireCount <= 0) {
			if (battle.m_ammunition >= this.m_ammoCost && battle.m_energy >= this.m_energyCost) {
				...
				var temp = new StaticProjectile (battle, this.m_x, this.m_y, this.m_angle);
				this.bullet.push (temp);
				
				g_staticSoundNode.Play(this.m_x, this.m_y);
			}
		}
	}
	...
	this.SpecialAttack = function () {
		if (rateOfFireCount <= 0) {
			if (battle.m_ammunition >= this.m_ammoCost && battle.m_energy >= this.m_energyCost) {
				...
				var temp = new EncaseStaticProjectile (battle, this.m_x, this.m_y, this.m_angle);
				this.bullet.push (temp);
				
				g_cannonSoundNode.Play(this.m_x, this.m_y);
			}
		}
	}
