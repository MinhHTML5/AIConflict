function ShockWaveTurret (battle, x, y) {
	...
	var baseImage = g_graphicEngine.LoadImage(ROOT_FOLDER + "Image/ActionPhase/Turret/ShockWave/Base.png");
	
	this.m_type = ENUM_TURRET_SHOCKWAVE;
	this.m_blockX = x;
	this.m_blockY = y;
	...
	this.m_name = g_turretData[ENUM_TURRET_SHOCKWAVE].m_name;
	this.m_range = g_turretData[ENUM_TURRET_SHOCKWAVE].m_range;
	this.m_rotateSpeed = g_turretData[ENUM_TURRET_SHOCKWAVE].m_rotateSpeed;
	this.m_rateOfFire = g_turretData[ENUM_TURRET_SHOCKWAVE].m_rateOfFire;
	this.m_ammoCost = g_turretData[ENUM_TURRET_SHOCKWAVE].m_ammoCost;
	this.m_energyCost = g_turretData[ENUM_TURRET_SHOCKWAVE].m_energyCost;
	...
	g_waveSoundNode.SetCameraHandler (battle.camera);
	...
	this.Attack = function () {
		if (rateOfFireCount <= 0) {
			if (battle.m_ammunition >= this.m_ammoCost && battle.m_energy >= this.m_energyCost) {
				...
				var temp = new ShockWaveProjectile (battle, this.m_x, this.m_y);
				this.bullet.push (temp);
				
				g_waveSoundNode.Play(this.m_x, this.m_y);
			}
		}
	}
	...
	this.SpecialAttack = function () {
		if (rateOfFireCount <= 0) {
			if (battle.m_ammunition >= this.m_ammoCost && battle.m_energy >= this.m_energyCost) {
				...
				var temp = new SlowShockWaveProjectile (battle, this.m_x, this.m_y);
				this.bullet.push (temp);
