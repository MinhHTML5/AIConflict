function MissileProjectile (battle, x, y, angle, enemy) {
	var image = g_graphicEngine.LoadImage(ROOT_FOLDER + "Image/ActionPhase/Projectile/Missile/Missile.png");
	...
	this.m_homingSpeed = g_bulletData[ENUM_PROJECTILE_MISSILE].m_homingSpeed;
					...
					for (var j=0; j<enemy.length; j++) {
						...
						if (range <= this.m_areaOfEffect) {
							if (tempEnemy2.m_kineticShield == true) {
								tempEnemy2.TakeDamage (this.m_damage * KINETIC_SHIELD_PENALTY_TO_PHYSIC_DAMAGE, this.m_armorPierce);
							}
							else {
								tempEnemy2.TakeDamage (this.m_damage, this.m_armorPierce);
							}
						}
					}
