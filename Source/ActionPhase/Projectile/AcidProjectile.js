function AcidProjectile (battle, x, y, angle) {
	var image = g_graphicEngine.LoadImage(ROOT_FOLDER + "Image/ActionPhase/Projectile/Acid/AcidParticle.png");
			...
			for (var i=0; i<enemy.length; i++) {
				...
				&&  tempEnemy.m_live == true ) {
					if (tempEnemy.m_kineticShield == false) {
						if (g_profile.upgrade[UPGRADE_ACID][UPGRADE_TIER_2] == true) {
							tempEnemy.BuffAcid(ACID_BUFF_DURATION, ACID_BUFF_DPS + UPGRADE_ACID_ADDITIONAL_TIME);
						}
						else {
							tempEnemy.BuffAcid(ACID_BUFF_DURATION, ACID_BUFF_DPS);
						}
					}
				}
			}
