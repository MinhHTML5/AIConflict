function Corvette (battle, type) {
	var SHIELD_IMAGE_SIZE = 100;
	var SHIELD_ACTUAL_SIZE = 95;
	var HP_BAR_LENGTH = 70;
	var HP_BAR_OFFSET = 10;
	
	var EFFECT_IMAGE_SIZE = 100;
	var EFFECT_IMAGE_EXPAND_SPEED = 0.3;
	
	
	var dataID = (type / 10) >> 0;
	var variant = dataID - 20;
	var tier = type % 10;
	
	var image = g_graphicEngine.LoadImage(ROOT_FOLDER + "Image/ActionPhase/Enemy/Corvette/" + variant + "/" + tier + ".png");
	...
	var effectImage;
	if (variant == 1) 		effectImage = g_graphicEngine.LoadImage(ROOT_FOLDER + "Image/ActionPhase/Enemy/Corvette/Heal.png");
	else if (variant == 2)	effectImage = g_graphicEngine.LoadImage(ROOT_FOLDER + "Image/ActionPhase/Enemy/Corvette/Shield.png");
	else if (variant == 3)	effectImage = g_graphicEngine.LoadImage(ROOT_FOLDER + "Image/ActionPhase/Enemy/Corvette/Shield.png");
	else if (variant == 4)	effectImage = g_graphicEngine.LoadImage(ROOT_FOLDER + "Image/ActionPhase/Enemy/Corvette/Speed.png");
	else if (variant == 5)	effectImage = g_graphicEngine.LoadImage(ROOT_FOLDER + "Image/ActionPhase/Enemy/Corvette/Invisible.png");
	...
	this.Update = function (deltaTime) {
		if (this.m_live == true) {
			...
			if (skillEffectImageSize >= 0) {
				skillEffectImageSize += EFFECT_IMAGE_EXPAND_SPEED * deltaTime;
				if (skillEffectImageSize >= this.m_range * 2) {
					skillEffectImageSize = -1;
				}
			}
	...
	this.Draw = function (context) {
		if (this.m_live == true) {
			...
			if (skillEffectImageSize > 0) {
				var alpha2 = 1 - (skillEffectImageSize / (this.m_range * 2));
				g_graphicEngine.SetDrawModeAddActive (context, true);
				g_graphicEngine.Draw (context, effectImage, 0, 0, EFFECT_IMAGE_SIZE, EFFECT_IMAGE_SIZE, (this.m_x >> 0) - offsetX - (skillEffectImageSize * 0.5) >> 0, (this.m_y >> 0) - offsetY - (skillEffectImageSize * 0.5) >> 0, skillEffectImageSize, skillEffectImageSize, alpha2, false, false);
				g_graphicEngine.SetDrawModeAddActive (context, false);
			}
		}
	}
	...
	this.UseSkill = function () {
		...
		for (var i=0; i<enemy.length; i++) {
			...
			if (range <= this.m_range) {
				if (variant == 1)		tempEnemy.Heal (CORVETTE_HEAL_AMOUNT * tier);
				else if (variant == 2)	tempEnemy.m_kineticShield = true;
				else if (variant == 3)	tempEnemy.m_energyShield = true;
				else if (variant == 4)	tempEnemy.BuffSpeed (CORVETTE_SPEED_BUFF_DURATION, CORVETTE_SPEED_BUFF_FACTOR);
				else if (variant == 5)	tempEnemy.BuffInvisible (CORVETTE_INVISIBLE_DURATION);
			}
		}
		
		skillEffectImageSize = 0;
	}
